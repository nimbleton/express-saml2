        ...
        base64LoginResponse: function base64LoginResponse(requestInfo, referenceTagXPath, entity, user, rcallback, rtnCallback) {
            ...
            if(metadata && metadata.idp && metadata.sp) {
                ...
                if(idpSetting.loginResponseTemplate) {
                    ...
                    //Build basic response attribute statement based on the attributes object of the passed in user
                    var attrXml = '<saml:AttributeStatement>';
                    Object.keys(user.attributes).forEach(function(attr,index){
                        attrXml += '<saml:Attribute NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:basic" Name="' + attr + '"><saml:AttributeValue xsi:type="xs:string">' + user.attributes[attr] + '</saml:AttributeValue>';
                    });
                    attrXml += '</saml:AttributeStatement>';
                    var tvalue = {
